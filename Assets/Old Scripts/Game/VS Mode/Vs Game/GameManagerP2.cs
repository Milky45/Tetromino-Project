...
public class GameManagerP2 : MonoBehaviour
{
    ...
    [Header("References")]
    [SerializeField] public BoardManagerP2 board;
    [SerializeField] public GameManager gameP1;
    ...
    public int chosenCharacter = 0; // debugging purposes
    public bool enableSpawn = true;
    public static float movementSensitivity = 0.10f; // Controls how quickly the piece responds to input
    ...
    private float lockoutDuration = 0.1f;// 100ms after hard drop
    ...
    public InputAction SkillAction => controls.VsModeP2.Skill;
    ...
    public void Start()
    {
        ...
        chosenCharacter = CharacterSelector.P2ChosenCharacter;
        movementSensitivity = P2Settings.chosenSens;
        ...
        AmmoBox.AmmoUpdateP2();
        AmmoBox.GrenadeUpdateP2();
        SpawnNextPiece();
    }
    ...
    public void Awake()
    {
        ...
        controls.VsModeP2.Attack.performed += ctx => TryAttack();
        controls.VsModeP2.EMPGrenade.performed += ctx => TryUseEmpGrenade();
        controls.VsModeP2.Pause.performed += ctx => TogglePause();
        controls.VsModeP2.Skill.performed += ctx => TryCharacterSkill();
    }
    ...
    public void SpawnNextPiece()
    {
        ...
        GameObject pieceObj = new GameObject("ActivePieceP2");
        PieceControllerP2 controller = pieceObj.AddComponent<PieceControllerP2>();
    ...
    public void TryHoldPiece(TetrominoData current, PieceControllerP2 controller)
    {
        if (isPaused) return;
        var activePiece = GameObject.Find("ActivePieceP2")?.GetComponent<PieceController>();
    ...
    public void ComboCount()
    {
        ...
        P2score = P2score + ((100 * scoreMultiplier) * linesCleared);
        ...
        if (linesCleared > 0)
        {
            ...
            // EMP Grenade reward on 4x combo or higher
    ...
    public void TryAttack()
    {
        ...
        if (attackAmmo > 0)
        {
            ...
            var opponentPiece = GameObject.Find("ActivePieceP1")?.GetComponent<PieceControllerP2>();
            if (opponentPiece != null)
            {
                opponentPiece.Clear(); // Clear its tiles temporarily
            }

            gameP1.ReceiveDeadLine();
    ...
    {
        isGameOver = true;
        vsGameOverManager.OnPlayer2Fail();
        ...
        // Optionally: Destroy any existing "ActivePiece"
        GameObject existingPiece = GameObject.Find("ActivePieceP2");
    ...
    private void ApplyDeadLine()
    {
        ...
        if (activePiece != null)
        {
            ...
            {
                activePiece.LockPiece(); // Try to move up if invalid position
            }
            else
            {
