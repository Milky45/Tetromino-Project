...
public class BoardManagerP2 : MonoBehaviour
{
    ...
    public void PushUp()
    {
        for (int y = boardSize.y - 2; y >= 0; y--) // bottom to top, excluding topmost
        {
            for (int x = -boardSize.x / 2; x < boardSize.x / 2; x++)
            {
                ...
                tilemap.SetTile(from, null); // This clears the old tile
            }
        }
    }
    ...
    private void ClearBottomDeadLines(int linesCleared)
    {
        ...
        {
            // Always clear the bottom-most row
            int y = -boardSize.y / 2;
    ...
    public void ScorchSkillClearBottomLines()
    {
        ...
        {
            // Check if the line has any tile at all
            ...
            for (int x = -boardSize.x / 2; x < boardSize.x / 2; x++)
            {
                ...
                if (tilemap.HasTile(pos))
                {
                    ...
                    if (tilemap.GetTile(pos) != tileTypes[7]) // Not a dead line tile
                    {
                        isDeadLine = false;
                    }
                }
