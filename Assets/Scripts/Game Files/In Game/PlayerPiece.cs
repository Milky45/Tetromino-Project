...
public class PlayerPiece : MonoBehaviour
{
    ...
    public void Start()
    {
        ...
        if (playerManager.playerStatus.isPlayer1)
        {
        ...
        //holdCallback = ctx => playerManager.TryHoldPiece(data, this);
    ...
    private void Update()
    {
        ...
        if (horizontalMove != Vector2Int.zero && repeatTimerLR >= playerManager.playerStatus.movementSensitvity)
        {
            if (playerManager.playerStatus.isInverted)
                horizontalMove *= -1;
        ...
        {
            if (repeatTimerDown >= (playerManager.playerStatus.movementSensitvity * 0.1f)) // Soft drop is 10x faster than horizontal movement
            {
                TryMove(Vector2Int.down);
                playerManager.audioManager.PlaySFX(playerManager.audioManager.moveClip);
                repeatTimerDown = 0f;
            }
        }
        else
    ...
    public void Set()
    {
        foreach (Vector2Int cell in cells)
        {
            ...
            playerBoard.main_tilemap.SetTile(tilePos, data.tile);
        }
    }
